...
5. **PR test plan verified** — Claude verifies every test plan item it can (file reads, command output, API checks) and checks them off in the PR body with brief results. Items requiring human judgment (UI appearance, real-device behavior, subjective quality) are left unchecked and called out explicitly for user review. If all items are verified, proceed to merge. If any need the user, pause and list what's left before merging.
